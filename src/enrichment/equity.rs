...
static EQUITY_RULES: OnceLock<PostingRules> = OnceLock::new();
...
impl Contract for EquityContract {
    ...
    async fn enrich(&self, tx: &RawTransaction) -> Result<AttributeBag, FbError> {
        ...
        let gross = qty * price;
        let commission = gross * dec!(0.0010);
        let stamp = gross * dec!(0.005);
        ...
        attrs.set_amount(AmountType::Net, gross + commission + stamp);
    ...
    fn posting_rules(&self) -> PostingRules {
        EQUITY_RULES.get_or_init(|| PostingRules {
